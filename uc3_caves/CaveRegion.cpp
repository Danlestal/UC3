...
CaveRegion::CaveRegion(Ogre::Vector3 globalVector, UberCube *cube): mGlobalVector(globalVector), mCube(cube), mRegionInitialized(false)
{}
...
{
	mRegionNeighbours[newExit] = neighbourRegion;
}

std::map<CubeFace, CaveRegion*> CaveRegion::GetNeighbours()
{
	return mRegionNeighbours;
}